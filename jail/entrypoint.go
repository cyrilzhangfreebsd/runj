...
func SetupEntrypoint(id string, init bool, argv []string, env []string, consoleSocketPath string, rfilePath string) (*exec.Cmd, error) {
	path := execSkipFifo
	if init {
		var err error
	...
	args := append([]string{id, path, rfilePath}, argv...)
